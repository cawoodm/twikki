...
### 2. Plugin registry: `tw.plugins.registry[]`

A new array on `tw.plugins.registry`, populated during boot, mirroring the shape of `tw.modules[]`:
...
New tiddler: `src/packages/base/$PluginsWidget.tid` (matching the `plugin-css-layer` convention of `.tid` over `.js` so a `# StyleSheet` section can join later if needed). Structurally a sibling of `src/packages/base/$ModulesWidget.js`.

The `# Code` section registers a macro and returns a markdown table:
...
| File | Change |
|---|---|
| `src/platform/twikki.platform.js` | Add pre-scan step; populate `tw.plugins.registry[]`; correlate `registerPlugin` to entries; capture init/start errors on entries. |
| `src/packages/base/$PluginsWidget.tid` | **New.** `# Code` section defines `tw.macros.core.plugins`. |
...
| `tests/unit/plugin-registry.test.js` | **New.** Test the pre-scan + correlation + compat logic against fixtures. |
