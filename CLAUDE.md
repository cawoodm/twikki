...
| `npm run test:e2e` | Playwright suite ([tests/e2e/](tests/e2e/)) booting the real app via the Vite dev server (pinned port 5180): every wired platform event, boot-progress events, plugin health, `?safemode` (extension packages skipped, base plugins still load) and the boot-halt compat dialog. |
...
**The no-plugin invariant.** The core modules must stand alone: with **zero plugins** TWikki still loads the shadow tiddlers, renders (escaped-plain-text markdown fallback), creates/edits/section-edits/validates/saves, navigates and searches. **`?safemode` is *not* zero-plugins** — it skips only the **extension** packages (`$ExtensionPackages`); the **base** package (`$CorePackages`) still loads, so its plugins (including `$BaseMarkdownPlugin`) run and markdown still renders. A plugin failure may cost shortcuts, the preview pane, the dirty dot, the rich boot dialog or drag-and-drop — never the ability to open a tiddler and save a fix. The `?safemode` boot is exercised by [tests/e2e/decomposition.spec.js](tests/e2e/decomposition.spec.js).
