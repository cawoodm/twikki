...
- **`init()`** — the core **code** modules are statically imported (bundled by Vite — see the `CORE_MODULES` list and `import`s at the top of the platform), so nothing is downloaded for them. Only the shadow-tiddler **data** module `core.defaults.json` is fetched from `<baseUrl>/modules/core.defaults.json` — `baseUrl` resolves from the `MODULE_URL` constant (default `https://cawoodm.github.io/twikki`, overridable in index.html; a `localhost`/`IP:port` host uses `location.origin`). Online it loads from the network, offline from the **service-worker precache**. There is no `localStorage` module cache; a one-time boot sweep drops legacy `/modules/*` keys.
