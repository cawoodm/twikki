...
- **`init()`** — resolves `baseUrl` from the `MODULE_URL` constant (can be overriden in index.html, default `https://cawoodm.github.io/twikki`); on a `localhost`/`IP:port` host it instead uses `location.origin` so the dev server serves local sources. There is no `?pUrl`/`?url` query override. It then fetches the hard-coded core modules listed in the `modulesToLoad` array, each from `<baseUrl>/modules/<name>` — online from the network, offline from the **service-worker precache** (which is revisioned per build). There is no separate `localStorage` module cache; a one-time boot sweep drops legacy `/modules/*` keys.
