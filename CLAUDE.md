...
- **`init()`** — resolves `baseUrl` from the `MODULE_URL` constant (can be overriden in index.html, default `https://cawoodm.github.io/twikki`); on a `localhost`/`IP:port` host it instead uses `location.origin` so the dev server serves local sources (around line 72 of `twikki.platform.js`). There is no `?pUrl`/`?url` query override and no `/settings.json`.urls.moduleUrl in localStorage key. It then fetches the hard-coded core modules listed in the `modulesToLoad` array (around line 86), each from `<baseUrl>/modules/<name>` (or returns cached copies from `localStorage`).
...
**Module versioning & the compatibility gate.** Each core module declares a `version` (its own API, semver) **and** a `platform` (the release it was built for) — both as `const` literals read statically (the parsed source already lives in the cached `/modules<name>` entry, so there is **no** separate version stamp and **no** auto-refetch on platform change). In `init()`, before any module is `eval`'d, `checkModuleCompat` parses each module's `const platform = '...'` and classifies it vs the running `VERSION` (caret: same major, `running >= built-for`) as **ok**, **warn** (same major but newer, or no `platform` field — overridable), or **block** (different major, or failed download — hard). Boot halts on any **block** or any *freshly-fetched* **warn** (a cached warn already booted before, so it boots again silently) and opens `showCompatDialog` — a self-contained native `<dialog>` (no `tw.ui`/theme CSS yet). It writes nothing until the user picks **Update & reload** (store the shown modules; disabled if any block) or **Keep current versions** (reload using the cached set, no write); the source URL is editable and saved to `/settings.json`. `fetchCoreModule` only hits the network on an empty cache or `?reload`/`?update`. See [docs/MODULES.md](docs/MODULES.md).

**Plugin metadata & the registry.** Plugin tiddlers tagged `$Plugin` can declare a `# Meta` section (parsed via `core.sections.getSection`) with `name`, `namespace`, `version`, `platform`, `author`, `description`, `homepage`. Before extension tiddlers run, `prescanPluginRegistry()` walks every `$Plugin` tiddler, parses `# Meta`, computes caret compat against `VERSION`, and (re)populates `tw.pluginRegistry[]` — a flat array of `{name, namespace, source, package, meta, compat, instance, error}` mirroring `tw.modules[]`. `tw.extensions.registerPlugin(ns, name, factory)` correlates each registered instance to its registry entry (or creates an orphan `exempt` entry on the fly), and `initPlugins`/`runPlugins` capture init/start throws onto `entry.error`. **The plugin compat gate is soft**: a `block` or `warn` row does NOT prevent the plugin from running — the surrounding try/catch already isolates failures from boot, and the `<<plugins>>` widget surfaces status so the user can decide. See [docs/PLUGINS.md](docs/PLUGINS.md) for the full developer reference; [`Plugins.tid`](src/packages/website/Plugins.tid) is the in-app overview.
...
`tw` is the global namespace. Subsystems hang off it: `tw.events` (pub/sub), `tw.tiddlers` (`.all`/`.visible`/`.trashed`), `tw.storage` (localStorage wrapper), `tw.run` (action API), `tw.ui`, `tw.core.*` (subsystem exports), `tw.macros`, `tw.plugins` (live instances by `[namespace][name]`), `tw.pluginRegistry` (flat informational array — see "Plugin metadata & the registry" above).
