...
| `npm test` | `node --test --watch ./tests/unit/*.test.js`. Coverage: the compile plugin ([tests/unit/compile-plugin.test.js](tests/unit/compile-plugin.test.js)), the section parser ([tests/unit/sections.test.js](tests/unit/sections.test.js)), parameter parsing ([tests/unit/params.test.js](tests/unit/params.test.js)), the design-token contract ([tests/unit/tokens.test.js](tests/unit/tokens.test.js)), the semver/compat helpers ([tests/unit/semver.test.js](tests/unit/semver.test.js), [tests/unit/module-compat.test.js](tests/unit/module-compat.test.js)) and the workspace-scoped store ([tests/unit/store.test.js](tests/unit/store.test.js)). |
| `npm run test:e2e` | Playwright suite ([tests/e2e/](tests/e2e/)) booting the real app via the Vite dev server (pinned port 5180): every wired platform event, boot-progress events, plugin health, `?safemode` (the no-plugin invariant) and the boot-halt compat dialog. |
...
**The no-plugin invariant.** With zero plugins, or under `?safemode`, TWikki must still load the shadow tiddlers, render (escaped-plain-text markdown fallback), create/edit/section-edit/validate/save, navigate and search. A plugin failure may cost shortcuts, the preview pane, the dirty dot, the rich boot dialog or drag-and-drop — never the ability to open a tiddler and save a fix. Guarded by [tests/e2e/decomposition.spec.js](tests/e2e/decomposition.spec.js).

**Boot progress.** The platform emits progress ticks (`init`/`fetch`/`compat`/`eval`/`modules-ready`/`package`/`plugins`/`ready`) via `bootProgress()`: buffered on `tw.tmp.bootProgress` **and** dispatched as a native `twikki.boot.progress` DOM CustomEvent (listenable from `index.html` before any TWikki infrastructure exists); once `core.js` brings up `tw.events`, the buffer is replayed onto the bus as `boot.progress` events.
